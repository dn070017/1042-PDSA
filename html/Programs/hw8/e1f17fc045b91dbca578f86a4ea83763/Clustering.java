 ...
 private static class setPoint implements Comparable <setPoint> { 
    ...
    public static void main(String[] args)throws Exception {
     ...
     try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) {
        ...
        Clustering[] cluster = new Clustering[N*100];
...
//          System.out.println(points[i].x()+""\t""+points[i].y());
        ...
        if(N<3){
            ...
            MaxPQ<setPoint> ans1 =new MaxPQ<setPoint>();
            for(int i=0;i<N;i++){
               ansstructure[i]=new setPoint(1,points[i].x(),points[i].y());
               ans1.insert( ansstructure[i]);
            }
            while(!ans1.isEmpty()){
               System.out.println(ans1.max().number+"" ""+String.format(""%.2f"",ans1.max().x)+"" ""+String.format(""%.2f"",ans1.max().y));
               ans1.delMax();}
            System.out.println( String.format(""%.2f"", points[0].distanceTo(points[1])));
        ...
        for (int i=0;i<N;i++){
            ...
            Clustering[] findsmallcluster = new Clustering[N*100];
        ...
        int nn=0;
        int t=0;
        Clustering[] tempcluster2 = new Clustering[N*100];
...
//                              StdDraw.clear();
        ...
        if(NN==3){
            ...
            setPoint[] ansstructure =new setPoint[N];
            Clustering[] finalcluster = new Clustering[N*10];
//            StdDraw.clear();
            int countclaster=0;
            int clastercount=0;
                for(int i=0;i<N;i++){
