...
//              {
//                  if(Cluster.get(i).IsClusted == false){ copy.add(record.get(i));}
//              }
...
//                {
//                    if(Cluster.get(b).cs.equals(copy.get(c).cs))
//                    {
//                        temp[a] = copy.get(c);a++;
//                    }
//                }
//            }
           ...
           Pair target = new Pair(temp[0],temp[1]);mp.insert(target);
                target = new Pair(temp[0],temp[2]);mp.insert(target);
                target = new Pair(temp[1],temp[2]);mp.insert(target);
                
                
                target =(Pair)mp.delMin(); 
//                if(target.clusters[0].size() < target.clusters[0].size())
//                {
//                    t1 = target.clusters[0];t2 = target.clusters[1];
//                }
//                else
//                {
//                   t2 = target.clusters[0];t1 = target.clusters[1]; 
//                }
                t1 = target.clusters[0];t2 = target.clusters[1]; 
                double min = 10;
                ...
                //小的放上層
                for(int i = 0; i < t1.allpoint.size() ;i++)
                {
                    for(int j = 0; j < t2.allpoint.size() ;j++)
                    {
                        if(min > t1.allpoint.get(i).distanceTo(t2.allpoint.get(j)))
                        {
                            min = t1.allpoint.get(i).distanceTo(t2.allpoint.get(j));
                        }
                    }
                }
