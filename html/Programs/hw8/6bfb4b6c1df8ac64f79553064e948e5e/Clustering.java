    ...
    public static void main(String[] args) throws Exception {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) {
            ...
            if (pointCount < 3) {   //小於三點的情況，不需做clustering
                ...
                MinPQ minpq = new MinPQ(pointcoord.length);  //第一次算全部的點
                ...
                while (pointcoord.length > 3) {
                    MinPQ temppq = new MinPQ(minpq.size());
                    int pqsize=minpq.size();
                    for (int i = 0; i < pqsize; i++) {
                        Event tempevent = (Event) minpq.delMin();
                        if (tempevent.point1.equals(temp.point1) != true && tempevent.point1.equals(temp.point2) != true && tempevent.point2.equals(temp.point1) != true && tempevent.point2.equals(temp.point2) != true) {
                            temppq.insert(tempevent);
                        }
                    }
                    minpq = temppq;
                    for (int i = 0; i < pointcoord.length - 1; i++) {
                        Event event = new Event(pointcoord[pointcoord.length - 1], pointcoord[i]);
                        minpq.insert(event);
                    }
                    temp = (Event) minpq.delMin();
