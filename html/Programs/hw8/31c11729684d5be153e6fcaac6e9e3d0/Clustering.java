...
public class Clustering {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int i =0;i < 3;i++){
                ...
                else if(size[i]<size[MIN])MIN = i;
                else {
                    if(x[i]<x[MAX])MAX = i;
                    else if(x[i]>x[MIN])MIN = i;
                    else{
                        if (y[i]<y[MAX])MAX = i;
                        else if(y[i]>y[MIN])MIN = i; }}}
        if((MAX==0&&MIN==1)||(MAX==1&&MIN==0))MIDDLE=2;
        else if((MAX==0&&MIN==2)||(MAX==2&&MIN==0))MIDDLE=1;
        else if((MAX==1&&MIN==2)||(MAX==2&&MIN==1))MIDDLE=0;
        System.out.println(size[MAX]+"" ""+String.format(""%.2f"",x[MAX])+"" ""+String.format(""%.2f"",y[MAX]));
        System.out.println(size[MIDDLE]+"" ""+String.format(""%.2f"",x[MIDDLE])+"" ""+String.format(""%.2f"",y[MIDDLE]));
        System.out.println(size[MIN]+"" ""+String.format(""%.2f"",x[MIN])+"" ""+String.format(""%.2f"",y[MIN]));
        for(int i = 0;i<N;i++){
            for(int j = i+1;j<N;j++){
