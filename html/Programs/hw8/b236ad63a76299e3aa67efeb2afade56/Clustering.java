...
public class Clustering {
    ...
    public static void main(String[] args) throws Exception {
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) {
            ...
            while (count > 3) {
                ...
                for (int i = 0; i < count - 1; i++) {
                    for (int j = i + 1; j < count; j++) {
                        double Dis = newClusters[i].distanceTo(newClusters[j]) ;
                        if (Dis < minDis){
                            minDis = Dis ;
                            min[0] = newClusters[i] ;
                            indPosition[0] = Position[i] ;
                            min[1] = newClusters[j] ;
                            indPosition[1] = Position[j];
                        }
                    }
                }
                ...
                int[] newPositionSave = new int[count - 1];
                sumNum = 0;
                for (int i = 0; i < count; i++) {
                    if (!newClusters[i].equals(min[0]) && !newClusters[i].equals(min[1])) {
                        newClustersSave[sumNum] = newClusters[i];
                        newWeightSave[sumNum] = newWeight[i];
                        newPositionSave[sumNum] = Position[i] ;
                        sumNum++;
                    }
                }
                ...
                newPositionSave[sumNum] = index ;
                newClusters = newClustersSave;
                newWeight = newWeightSave;
                Position = newPositionSave ;
            ...
            System.out.println(String.format(""%.2f"", pq.delMin().Dis));
        }
    }
