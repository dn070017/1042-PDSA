...
public class Clustering{
    ...
    public static class Cluster implements Comparable<Cluster> {
        ...
        int count;
        int number_gen=0;
        //public static final Comparator<Cluster> iDorder = new iDorder();
        ...
        // private class PolarOrder implements Comparator<Cluster>
        public Comparator<Cluster> iDorder() {
        return new IDorder();
    }
        private class IDorder implements Comparator<Cluster> {
            //@Override
            public int compare(Cluster q1, Cluster q2){
            if(q1.number_gen > q2.number_gen) return -1;
            if(q1.number_gen < q2.number_gen) return 1;
    ...
    public static void main(String[] args) throws Exception{
        ...
        while(cluster.size()>3){
            ...
            for (int i = 0; i < cluster.size(); i++){
                //c.distanceTo(cluster.get(i));
                if(c!=cluster.get(i)&&cluster.get(i).count==0) pq.insert(new Pair(c,cluster.get(i)));
            }
        ...
        //Collections.sort(cluster,iDorder());
            ...
            /*while(!pq.isEmpty()){
                Pair e =pq.delMin();
                if(pq.size()<3)System.out.printf(""%d %.2f %.2f\n"",e.,cluster.get(i).cx,cluster.get(i).cy);
            }*/
