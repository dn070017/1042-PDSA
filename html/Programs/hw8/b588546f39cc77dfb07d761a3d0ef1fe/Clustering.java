...
public class Clustering {
    ...
    private static class Cluster implements Comparable<Cluster> {
        ...
        public Cluster(double x, double y, int size, int index, int head, int position) {
            this.x = x;
            this.y = y;
            this.size = size;
            this.index = index;
    ...
    public static void main(String[] args) throws Exception {
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) {
            ...
            Cluster[] queue = new Cluster[number];
            ...
            while ((stillhave = br.readLine()) != null) {
                ...
                wei[id] = new Cluster(Double.parseDouble(temp[0]), Double.parseDouble(temp[1]), 1, id, id, id);
                queue[id] = wei[id];
                id++;
            }
            ...
            while (clusternum > 3) {
                ...
                if (a.a.valid == 1 && a.b.valid == 1) {
                    ...
                    wei[id] = new Cluster(a.a.centriodx(), a.a.centriody(), a.a.getSize(), id, a.a.getHead(), a.a.getPosition());
                    wei[id].merge(a.b);
                    queue[a.a.getPosition()] = wei[id];
                    //queue[a.b.getPosition()] = wei[clusternum-1];
                    for (int i = 0; i < number; i++) {
                        if (i != wei[id].getPosition()) {
                            Pair test = new Pair(queue[i], queue[wei[id].getPosition()], i, id);
                            pq.insert(test);
                        }
                    }
                    id++;
            ...
            for (int i = 0; i < 3; i++) {
                ...
                String s1 = String.format(""%.2f"", mpq.max().centriodx());
                String s2 = String.format(""%.2f"", mpq.max().centriody());
                System.out.println(mpq.delMax().size + "" "" + s1 + "" "" + s2);
            }

            String s3 = String.format(""%.2f"", ans.getDistance());
