...
public class Cluster implements Comparable<Cluster>{
    ...
    public Cluster(double x1, double y1, int id1){
        x=x1;
        y=y1;
        size=1;
        id=id1;
    }
    public Cluster(Cluster a, Cluster b){//丟兩個cluster進來，這個建構子可以直接merge他們
    ...
    public static void main(String[] args)  throws Exception {
        try(BufferedReader br = new BufferedReader(new FileReader(args[0]))){
            ...
            Cluster[] gg=new Cluster[2*N];
            for(int i=0;i<N;i++){
                String[] temp=br.readLine().split("" "");
                gg[i]=new Cluster(Double.parseDouble(temp[0]),Double.parseDouble(temp[1]),i);
            }
            ...
            */
            ...
            while(N-mixtime>3){
                ...
                gg[N+mixtime]=new Cluster(min[0],min[1]);
            ...
            //輸出三個cluster
            Cluster[] threesmall=new Cluster[3];
...
class cp implements Comparable<cp>{
    Cluster a,b;
    double dis;
    public cp(Cluster x, Cluster y){
        a=x;
        b=y;
        dis=x.distanceTo(y);
    }
    public Cluster[] pop(){
        Cluster haha[]=new Cluster[2];
        haha[0]=a;
        haha[1]=b;
        return haha;
    }    
