        ...
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) {
            ...
            if (pointCount < 3) {   //小於三點的情況，不需做clustering
                ...
                while (pointcoord.length > 3) {
                    MinPQ temppq=new MinPQ();
                    for (int i = 0; i < minpq.size(); i++) {
                        Event event = (Event) minpq.delMin();
                        if (event.point1.equals(temp.point1) || event.point1.equals(temp.point2) || event.point2.equals(temp.point1) || event.point2.equals(temp.point2)) {                            
                        } else {
                            temppq.insert(event);
                        }
                    }
