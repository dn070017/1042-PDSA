...
public class Clustering {
    ...
    public static void main(String[] args) throws Exception {
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) {
            ...
            int[] merged = new int[number * number];

            Cluster[] wei = new Cluster[number * number];
            QuickFindUF yuru = new QuickFindUF(number);
            String stillhave;
            ...
            while (clusternum > 3) {
                a = pq.delMin();
                if (merged[a.getIndex1()] + merged[a.getIndex2()] == 0) {
                    merged[a.getIndex1()] = 1;
                    merged[a.getIndex2()] = 1;
            ...
            for (int i = 0; i < 3; i++) {
                b = mpq.delMax();
                String s1 = String.format(""%.2f"", b.centriodx());
                String s2 = String.format(""%.2f"", b.centriody());
                System.out.println(b.getSize() + "" "" + s1 + "" "" + s2);
            }
            Pair f;
            f = pq2.min();
            String s3 = String.format(""%.2f"", f.getDistance());
