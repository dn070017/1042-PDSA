...
public class Clustering{
	...
	public static void main(String[] args) throws Exception{
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))) {
			...
			if(N > k)clusterArray = new Cluster[N + N - k];
			else     clusterArray = new Cluster[N];
			if(N > k)Exist = new boolean[N + N - k];
			else     Exist = new boolean[N];
				...
				//if(clusterArray[c.p_idx].exist && clusterArray[c.q_idx].exist){
					...
					for(int i = 0; i < id_max-1; i++){
						...
						//if(clusterArray[i].exist){
							//System.out.printf(""Insert %2d %2d\n"", i, id_max);
							if(i == id_max)System.out.println(""!!"");
	...
	public static void Print_out(int k){
		...
		for(int i = 0; i < k; i++){
			c_out[i] = clusterArray[clusterArray.length - 1 - i];
		}
