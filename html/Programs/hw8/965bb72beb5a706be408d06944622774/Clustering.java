...
.
        ...
        public int compareTo(Cc that) {
            ...
            for (int i = 0; i < n + minus / 2; i++) {
                if(pointss[that.place].x()==-1){
                                return 1;
                }
                if (pointss[this.place].x() == -1) {
                    return 0;
                }
                    if (points[this.place].distanceTo(points[i]) < a && this.place != i&&pointss[i].x() != -1) {
                        a = points[this.place].distanceTo(points[i]);
                        outa=i;
                    }
                    if (points[that.place].distanceTo(points[i]) < b && that.place != i&&pointss[i].x() != -1) {
...
//          初始化pq
            ...
            double a=1;
                        //找距離最小
                for (int i = 0; i < n + minus / 2; i++) {for (int k = 0; k < minus; k++) {if (pointss[i].x() == -1) {i++;k = -1;} else {break;}}if (points[mem1].distanceTo(points[i]) < a && points[mem1].distanceTo(points[i]) != 0) { a = points[mem1].distanceTo(points[i]); b = i;}}  minus++;mem2 = b;
...
//                points[n + minus / 2].draw();
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if(mess[out[0]]==mess[out[1]]||mess[out[2]]==mess[out[1]]||mess[out[0]]==mess[out[2]]){
                                for (int j = i; j < 3; j++) {
                    if (points[out[i]].x() > points[out[j]].x()&&mess[out[i]]==mess[out[j]]) {
                        change = out[i];
                        out[i] = out[j];
                        out[j] = change;
                    }
                    if (points[out[i]].y() > points[out[j]].y()&&points[out[i]].x() == points[out[j]].x()) {
                        change = out[i];
                        out[i] = out[j];
                        out[j] = change;
                    }
