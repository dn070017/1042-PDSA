...
//                                         BFind++;            
                    ...
                    if(AFind==-1 || BFind==-1)
                      ...
                      //StdDraw.line(clus[AFind].Centroid.x(),clus[AFind].Centroid.y(),clus[BFind].Centroid.x(),clus[BFind].Centroid.y());
...
//            }
                            ...
                            //System.out.println(clus[0].getSize());
//                int JItem=0;
//                for(int k=0;k<ClusNum; k++){
//                    for(int i=0;i<clusF[k].getSize();i++){
//                        if(JItem<ClusNum){
//                        for(int j=0;j<clusF[JItem++].getSize();j++){
//                            Double distance = Math.sqrt(Math.pow(clusF[0].point[i].x() - clusF[1].point[j].x(), 2) + Math.pow(clusF[0].point[i].y() - clusF[1].point[j].y(), 2));
//                   pqF.insert(distance);
//                   }
//                        }
//                    }
//                }
   if(ClusNum>=3){
           for(int i=0;i<clusF[0].getSize();i++){
               for(int j=0;j<clusF[1].getSize();j++){
                   Double distance = Math.sqrt(Math.pow(clusF[0].point[i].x() - clusF[1].point[j].x(), 2) + Math.pow(clusF[0].point[i].y() - clusF[1].point[j].y(), 2));
                   pqF.insert(distance);
               }
           }
           for(int i=0;i<clusF[0].getSize();i++){
               for(int j=0;j<clusF[2].getSize();j++){
                   Double distance = Math.sqrt(Math.pow(clusF[0].point[i].x() - clusF[2].point[j].x(), 2) + Math.pow(clusF[0].point[i].y() - clusF[2].point[j].y(), 2));
                   pqF.insert(distance);
               }
           }
           for(int i=0;i<clusF[1].getSize();i++){
               for(int j=0;j<clusF[2].getSize();j++){
                   Double distance = Math.sqrt(Math.pow(clusF[1].point[i].x() - clusF[2].point[j].x(), 2) + Math.pow(clusF[1].point[i].y() - clusF[2].point[j].y(), 2));
                   pqF.insert(distance);
               }
           }
   }
   ...
   {
       Double distance = Math.sqrt(Math.pow(clusF[0].point[0].x() - clusF[1].point[0].x(), 2) + Math.pow(clusF[0].point[0].y() - clusF[1].point[0].y(), 2));
                   pqF.insert(distance);
   }
   else
       pqF.insert(0.00);
