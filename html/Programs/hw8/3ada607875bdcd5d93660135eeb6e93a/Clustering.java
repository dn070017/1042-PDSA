...
public class Clustering {
    ...
    private static class cluster implements Comparable<cluster> {
       ...
       public cluster(int id,double rx, double ry, int size,double dis){
          this.rx= rx;
          this.ry= ry;
          this.size=size;
          this.dis=dis;
  ...
  public static void main(String[] args) throws Exception {
        try(BufferedReader br = new BufferedReader(new FileReader(""input8-0.txt""))){
            ...
            Clustering.cluster[] fullp = new Clustering.cluster[N];
                ...
                String[] p = in.split(""\\s+"");
                fullp[idx] = new Clustering.cluster(idx,Double.parseDouble(p[0]),Double.parseDouble(p[1]),1,1.5);   
                idx++;
                }
            ...
            //StdOut.println(minid);//die
            //StdOut.println(minid2);//grow
            //j改成新的point
            ...
            for (int i=0;i<ON;i++){
               if(fullp[i].size > 0 && minid2!=i){//update the new cluster 
                   ...
                   if (fullp[i].nid==fullp[minid].id||fullp[i].nid==fullp[minid2].id){//once nid is dead
                      fullp[i].dis=2;
                      for(int j=0;j<ON;j++){
                      ...
                      if(fullp[i].ndis(fullp[minid2])<fullp[i].dis) 
