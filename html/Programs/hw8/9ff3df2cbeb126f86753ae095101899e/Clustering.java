    ...
    public static void main(String[] args) throws Exception{
        ...
        while(pointCount<edge){//double douNumber1=Double.parseDouble(number.toString());
            ...
            for(int i = 0;i < pointCount;i++){
                ...
                if(alocation!=-1 && blocation!=-1)break;}
            ...
            if(clustering.find(alocation) != alocation || clustering.find(blocation) != blocation){ //自己是自己的根  沒有被MERGE過
                continue;}
            for(int i = 0;i < N;i++){
                int temp = clustering.find(i);
                if(temp == alocation)acount++;
                else if(temp == blocation)bcount++;}//計算總共有幾個原始的小孩 for算重心
            ...
            for(int i = 0;i < (pointCount-1) ;i++){
                int temp = clustering.find(i);
                if(temp==pointCount-1)continue;
                if(temp!=i)continue;
                double dd = Math.sqrt(Math.pow(clustering.clusterArray[pointCount-1].x()-clustering.clusterArray[temp].x(),2)+Math.pow(clustering.clusterArray[pointCount-1].y()-clustering.clusterArray[temp].y(),2));
        ...
        for(int i =0;i < N;i++){
            int temp = clustering.find(i);
            if(temp!=alocation && temp!=blocation)
            {clocation = temp;
            break; }}
        for(int i =0;i < N;i++){
            int temp = clustering.find(i);
            if(temp==alocation)acount++;
            else if(temp==blocation)bcount++;
            else ccount++;}
       ...
       for(int i = 0;i<N;i++){
           int temp = clustering.find(i);
            for(int j = i+1;j<N;j++){
                if(temp == clustering.find(j))continue;
                else{
                    double dsq = Math.pow(clustering.clusterArray[i].x()-clustering.clusterArray[j].x(),2)+Math.pow(clustering.clusterArray[i].y()-clustering.clusterArray[j].y(),2);
                    double d = Math.pow(dsq,1/2.0);
