...
public class Player implements Comparable<Player>{ 
    ...
    public int compareTo(Player that) {
        System.out.println(TwoPair(that.cards)[0]);
...
.
    ...
    public int[] Straight(Card[] aa) {
        ...
        GP[0] = GP[0]/5;//save whether set is establish?
    ...
    public int[] Flush(Card[] cc) {
        Arrays.sort(cc);
         int[] GP = new int[2];
        for(int i=0; i<4; i++){
           if((GetPoint(cc[i])%10)-(GetPoint(cc[i+1])%10)==0)GP[0] = GP[0]+1;
        } 
    ...
    public int[] TwoPair(Card[] dd) {
        ...
        if(count==2){
            GP[0]=1;GP[1]=GetPoint(dd[temp[1]]);}
        else{ GP[0]=0;GP[1]=0;
        }
        return GP;
    }
    ...
    public int[] OnePair(Card[] ee) {
        ...
        if(count==1){
            GP[0]=1; GP[1]=GetPoint(ee[temp[0]]);}
        else{GP[0]=0; GP[1]=0;            
        }
        return GP;
    }
