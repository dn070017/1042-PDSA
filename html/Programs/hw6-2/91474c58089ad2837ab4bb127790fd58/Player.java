...
public class Player implements Comparable<Player>{ 
    ...
    public double getPlayerValue(Player p)
    {
        ...
        while(i < 4)
        {
            ...
            {
                c1 = c1 + 2 + getFaceValue(p.cards[i].getFace()) * 0.000001;
                if(i == 4) break;
                if(p.cards[i].getFace().equals(p.cards[++i].getFace()))
                {
        ...
        if(c1 < 4.7)
        {
            ...
            if(k == 4)
            {
                c1 = 4.5 + getFaceValue(p.cards[4].getFace()) * 0.01 + getSuitValue(p.cards[4].getSuit()) * 0.001;
            }
            else if(k == 3 && getFaceValue(p.cards[0].getFace()) == 2 && getFaceValue(p.cards[4].getFace()) == 14)
            {
                c1 = 4.4 + getFaceValue(p.cards[3].getFace()) * 0.01 + getSuitValue(p.cards[3].getSuit()) * 0.001;
            }
        }
