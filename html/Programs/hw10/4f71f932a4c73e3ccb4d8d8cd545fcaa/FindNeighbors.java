...
public class FindNeighbors {
    ...
    public void init(Point2D[] points){
        if(points==null) return;//如果傳進來沒有點
        ...
        while(x!=null){
            if(x.getDirection()==true){//x是垂直
                if(p.x()<=x.getXValue()) {//如果新點在x的左邊
                    if(x.getLeft()!=null) x=x.getLeft();
                    else x.setLeft(new Node(p,false));//左邊沒有值，代表p必須長在那裏
                }
                else{//如果新點在x的右邊
                    if(x.getRight()!=null) x=x.getRight();
                    else x.setRight(new Node(p,false));//右邊沒有值，代表p必須長在那邊
                }
            }
            else {//x是水平
                if(p.y()<=x.getYValue()){//如果新點在x的下面
                    if(x.getLeft()!=null) x=x.getLeft();
                    else x.setLeft(new Node(p,true));//下面沒有值，代表p必須長在那邊
                }
                else{//如果新點在x的上面
                    if(x.getRight()!=null) x=x.getRight();
                    else x.setRight(new Node(p,true));//上面沒有值，代表p必須長在那邊
                }
            }
        }
    }
    ...
    private void findQuery(Node n){
        ...
        if(thisNode.leftRight(n)==-1){//p點在n的左邊
            findQuery(n.getLeft());
            if(thisNode.lineDistance(n)<p.distanceTo(pq.max().getValue())||pq.size()<z) findQuery(n.getRight());
            return;
        }
        else{//p點在n的右邊
            findQuery(n.getRight());
            if(thisNode.lineDistance(n)<p.distanceTo(pq.max().getValue())||pq.size()<z) findQuery(n.getLeft());
            return;
        }       
    }
}
...
class Node{
    ...
    public int leftRight(Node n){
        if(n.getDirection()==true){//比較對象是垂直線
            if(this.getXValue()<n.getXValue()) return -1;//比x
            else if(this.getXValue()==n.getXValue()) return 0;
            return 1;
        }
        else{//比較對象是水平線
            if(this.getYValue()<n.getYValue()) return -1;//比y
            else if(this.getYValue()==n.getYValue()) return 0;
            return 1;
        }
    }
