...
public class FindNeighbors {
  ...
  public void insert(Point2D p) 
    ...
    private void searchNearest(Node node, Point2D p, int k ) 
    {
        ...
        mpq.insert(pa);
        if(pa.compareTo((Pair)mpq.max()) == -1 && mpq.size() > k )
        {
            mpq.delMax();
        }
        ...
        if (node.left != null && node.right != null) 
        {
            ...
            if (left < right) 
            {
                pa  = new Pair(node.right.value,right);
                mpq.insert(pa);
                if(pa.compareTo((Pair)mpq.max()) == -1 && mpq.size() > k )
                {
                    mpq.delMax();
                ...
                if(node.size >3 && node.right != null && node.left !=null)
                {
                    if( node.right.right == null && node.right.left == null)
                    {
                        searchNearest(node.left.left, p,k);
                    }
                    if(node.left.left==null && node.left.right == null)
                    {
                        searchNearest(node.right.right,p,k);
                    }
                }
            ...
            {
                pa  = new Pair(node.left.value,left);
                mpq.insert(pa);
                if(pa.compareTo((Pair)mpq.max()) == -1 && mpq.size() > k )
                {
                   mpq.delMax();
                ...
                previous = node;
                if(judge(node.right,previous,p))
                {
                    node.left = null;
                }
                searchNearest(node.right, p,k);
    ...
    private boolean judge(Node now ,Node pre, Point2D target)
    {
     ...
     public static void main(String[] args)throws Exception {
           ...
           double [][] vertices = new double[Totalpoints][2]; AllPoint = new Point2D[Totalpoints];//Cluster = new cluster[Totalpoints];
            ...
            Point2D target = new Point2D(0.7281417757,0.7145077083);
            Point2D [] ans = new Point2D[1];
            FindNeighbors fn = new FindNeighbors();
            fn.init(AllPoint);
            ans = fn.query(target, 1);
