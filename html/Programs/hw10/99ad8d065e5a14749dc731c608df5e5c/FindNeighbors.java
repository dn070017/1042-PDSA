...
//        StdDraw.point(point.x(),point.y());
    ...
    public void near(Node start ,int rank) {
        ...
        if(start.left != null){//3. going left if there is one 
        near(start.left,rank);
        }
        
        
        if(start.right != null){// 4. check if we should go right
            Node temp = NearPoints.delMax();
            
            if(start.flag == 1){ //4-1
                if(temp.key.distanceTo(refer)< Math.abs(start.key.x()-refer.x())){
                    if(NearPoints.size()<rank-1){
            ...
            else {NearPoints.insert(temp); return;}
.
