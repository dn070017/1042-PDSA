    ...
    private Node findNearest(Node x, Point2D p, Orientation orientation,int k) {
       ...
       Node closet = x;
        ...
        if (first != null) {    
            ...
            if(pq.size()> k && record_flow!=null){
                
                if(orientation == Orientation.LeftRight && Math.abs(p.x()-x.p.x())>record_flow.dist){
                    return closet;
                }
                else if(orientation == Orientation.AboveBelow && Math.abs(p.y()-x.p.y())>record_flow.dist){
                    return closet;
                }
            }  
