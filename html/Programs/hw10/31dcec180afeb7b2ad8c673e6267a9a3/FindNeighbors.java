        ...
        if (!odd) {
            ...
            if (target.x() < run.setpoint.x()) {
                ...
                if (set.size() >= num) {
                    ...
                    Node x = set.delMax();
                    if ((target.distanceTo(x.setpoint))*(target.distanceTo(x.setpoint)) < (target.x() - run.setpoint.x())*(target.x() - run.setpoint.x())) {
                        set.insert(x);
                        return;
                    }
                    set.insert(x);
            ...
            if (target.y() < run.setpoint.y()) {
                ...
                if (set.size() >= num) {
                    ...
                    Node x = set.delMax();
                    if ((target.distanceTo(x.setpoint))*(target.distanceTo(x.setpoint)) < (target.x() - run.setpoint.y())*(target.x() - run.setpoint.y())) {
                        set.insert(x);
                        return;
                    }
                    set.insert(x);
