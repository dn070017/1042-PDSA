public class FindNeighbors {
    ...
    public Point2D[] query(Point2D point, int k){
        searchpath = new LinkedStackOfStrings();
        ...
        while(!searchpath.isEmpty()){
            ...
            if(pq.size() < k){                
                if(tempx == null){
                    GatherAll(x,point,pq,relation);
                    tempx = x;
                }
                else if(tempx==x.getLeft() && x.getRight()!=null){  
                    double tempd = Math.sqrt(Math.pow(x.getValue().x()-point.x(), 2)+Math.pow(x.getValue().y()-point.y(), 2));
                    pq.insert(tempd);
                    relation.put(tempd, x.getValue());
                    GatherAll(x.getRight(),point,pq,relation); 
                    tempx = x;
                }
                 else  if(tempx==x.getRight() && x.getLeft()!=null){
                    double tempd = Math.sqrt(Math.pow(x.getValue().x()-point.x(), 2)+Math.pow(x.getValue().y()-point.y(), 2));
                    pq.insert(tempd);
                    relation.put(tempd, x.getValue());
                    GatherAll(x.getLeft(),point,pq,relation); 
                    tempx = x;
                }
                 else {                    
                    double tempd = Math.sqrt(Math.pow(x.getValue().x()-point.x(), 2)+Math.pow(x.getValue().y()-point.y(), 2));
                ...
                double tempd = Math.sqrt(Math.pow(x.getValue().x()-point.x(), 2)+Math.pow(x.getValue().y()-point.y(), 2));
                if(tempd < pq.max() && relation.get(tempd)==null){//System.out.println(""HI"");
                            pq.delMax();
                            pq.insert(tempd);
                            relation.put(tempd, x.getValue());}
                if(x.getLevel()%2==0){
                    if(Math.abs(point.x()-x.getValue().x())<pq.max()){
                            if(tempx == x.getLeft() && x.getRight()!=null){
                                searchpath.push(x.getRight());tempx = x;
                            }
                            else if(tempx == x.getRight() && x.getLeft()!=null){
                                searchpath.push(x.getLeft());tempx = x;
                            }
                            else{
                                if(x.getLeft()!=null)searchpath.push(x.getLeft());
                                if(x.getRight()!=null)searchpath.push(x.getRight());
                            }
                        }}
                else{
                        if(Math.abs(point.y()-x.getValue().y())<pq.max()){
                            if(tempx == x.getLeft() && x.getRight()!=null){
                                 searchpath.push(x.getRight());tempx = x;
                            }
                            else if(tempx == x.getRight() && x.getLeft()!=null){
    ...
    public void GatherAll(Node x,Point2D point,MaxPQ<Double> pq,RelationBST<Double,Point2D> relation){
        double tempd = Math.sqrt(Math.pow(x.getValue().x()-point.x(), 2)+Math.pow(x.getValue().y()-point.y(), 2));
        pq.insert(tempd);//System.out.println(x.getValue().x()+"" ""+x.getValue().y());
        relation.put(tempd, x.getValue());
        if(x.getLeft()!=null)GatherAll(x.getLeft(),point,pq,relation);
        if(x.getRight()!=null)GatherAll(x.getRight(),point,pq,relation);
    }
    ...
    public static void main(String[] args) throws Exception{
        ...
        Point2D[] result = find.query(C,13);
