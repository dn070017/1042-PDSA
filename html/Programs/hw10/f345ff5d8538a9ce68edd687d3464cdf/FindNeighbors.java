...
public class FindNeighbors {
    ...
    public Node recur(Node x) {
        ...
        if ((x.deep % 2) == 0) {
            if (points[x.value].x() >= d.x()) {
                ...
                recur(x.left);
                if (sqrt((points[x.value].x() - d.x())) <= sqrt(dd[large - 1].distanceTo(d))) {
                    recur(x.right);
                } else {
                    return x;
                }
            ...
            if (points[x.value].y() >= d.y()) {
                if (points[x.value].distanceTo(d) <= dd[large - 1].distanceTo(d)) {//往下
                    dd[large - 1] = points[x.value];
                    Arrays.sort(dd, d.DISTANCE_TO_ORDER);
                }
                recur(x.left);
                if (sqrt((points[x.value].y() - d.y())) <= sqrt(dd[large - 1].distanceTo(d))) {
                    recur(x.right);
                } else {
                    return x;
                }
    ...
    public Point2D[] query(Point2D point, int k) {
        ...
        recur(root);
        result = dd;
        Arrays.sort(result, d.DISTANCE_TO_ORDER);
...
//            p.init(pointss);
//            Point2D ddd = new Point2D(0.3833339428 ,0.1459115606);
//            p.query(ddd, 3);
//
//        }
//    }
