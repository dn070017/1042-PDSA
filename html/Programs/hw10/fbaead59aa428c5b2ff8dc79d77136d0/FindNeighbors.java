...
public class FindNeighbors {
    ...
    public Point2D[] query(Point2D point, int k) {
        ...
        //MaxPQ violatepq = new MaxPQ();  //用來存所計算的距離
        Point2D[] result = new Point2D[k];
        Point2D[] result1 = new Point2D[k];
        qeurynum = k;
        distance(this.tree_2d, point, maxpq);
        //violate(this.tree_2d, point, violatepq);
        ...
        //int cc=violatepq.size();
//        int idx=k-1;
//        for (int i = 0; i <cc; i++) {
//            if(violatepq.size()<=k){
//                Node tt = (Node) violatepq.delMax();
//            temp1[idx] = tt.distance;
//            result1[idx] = tt.point;
//            idx--;
//            }else{
//                violatepq.delMax();
//            }
...
//        System.out.println("" "");
//        for (int i = 0; i < k; i++) {
//            System.out.println(result1[i].x() + "" "" + result1[i].y() + "" "" + temp1[i]);
//        }
        return result;
    }
    
    private void violate(Node x, Point2D point, MaxPQ<Node> q) {
        if (x == null) {
            return;
        }    
...
//            }
            ...
            a.init(points);
            Point2D[] result = new Point2D[100000];
            Point2D target;
