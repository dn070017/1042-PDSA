...
public class FindNeighbors {
    ...
    private void findNearest(Node x, Point2D p, int k, String orientation, MaxPQ<Event> pq) {
        ...
        if (left != null && left.rect.distanceSquaredTo(p) < pq.max().distance) {
            findNearest(left, p, k,nextorientation, pq);
        }
        if (right != null && right.rect.distanceSquaredTo(p) < pq.max().distance) {
    ...
    /*public static void main(String[] args) throws Exception {
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))){
            Point2D[] test = new Point2D[20000];
            int i = 0;
            ...
            test2.init(test);
            Point2D point = new Point2D(0.80867809,0.05794378);
