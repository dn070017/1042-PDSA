...
public class FindNeighbors {
    ...
    private Node findNearest(Node x, Point2D p, Orientation orientation,int k) {
       ...
       Node closet = x;
        ...
        if (first != null) {    
            ...
            if(pq.size()>=k){
                ...
                if(orientation == Orientation.LeftRight && Math.abs(p.x()-x.p.x())>closet.dist){
                    //StdOut.println(""@@@``````````@@@@@@@@@ ""+x.p+""dist""+x.dist)p.x()-x.p.x();
        ...
        if (second != null) {
            //closet=findNearest(second, p,nextOrientation, k);
            findNearest(second, p,nextOrientation, k);
            }
