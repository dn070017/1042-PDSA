...
public class FindNeighbors {
     ...
     protected static KdTree copykt = new KdTree();
    ...
    public Point2D[] query(Point2D point, int k){
        ...
        if(k >= mpq.size())
        {
            if(kt.minPoint.x() < AllPoint[0].x())
            {
                copykt.root = copykt.root.right;
                copykt.root.left = null;
                copykt.nearest(point,k);
            }
            else
            {
                copykt.root = copykt.root.left;
                copykt.root.right = null;
                copykt.nearest(point,k);
            }
        }
    ...
    private boolean Go(Node next ,Point2D t)
    {
     ...
     public static void main(String[] args)throws Exception {
           ...
           double [][] vertices = new double[Totalpoints][2]; AllPoint = new Point2D[Totalpoints];//Cluster = new cluster[Totalpoints];
            ...
            Point2D [] ans = new Point2D[9];
            FindNeighbors fn = new FindNeighbors();
            fn.init(AllPoint);
            ans = fn.query(target, 9);
