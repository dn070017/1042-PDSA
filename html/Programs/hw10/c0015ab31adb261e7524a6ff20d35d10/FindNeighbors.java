...
public class FindNeighbors {
    ...
    private void doNearest(Node node, Point2D queryPoint, int k) {
        ...
        double dis = nearestPointCandidate.distanceSquaredTo(queryPoint);
        ...
        if(maxPQ.size()==k)
        {
            ...
            if(node.vertical==true)
            {
                ...
                if(queryPoint.x() - node.p.x() > tmp.distance)
                {
                    doNearest(node.right, queryPoint, k);
                }
                else if(node.p.x() - queryPoint.x() > tmp.distance)
                {
                    doNearest(node.left, queryPoint, k);
                }                
                else
                {
                    if(queryPoint.x() - node.p.x() > 0){
                        doNearest(node.right, queryPoint, k);
                        doNearest(node.left, queryPoint, k);						
                    }
                    else{
            ...
            {
                if(queryPoint.y() - node.p.y() > tmp.distance)
                {
                    doNearest(node.right, queryPoint, k);
                }
                else if(node.p.y() - queryPoint.y() > tmp.distance)
                {
                    doNearest(node.left, queryPoint, k);
                }
