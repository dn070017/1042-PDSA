    ...
    public Point2D[] query(Point2D point, int k) {
        ...
        if (P.length <= 50000) {
            for (int i = 0; i < P.length; i++) {
                if ((P[i].x() <= point.x() + 0.4 && P[i].x() >= point.x() - 0.4) && (P[i].y() <= point.y() + 0.4 && P[i].y() >= point.y() - 0.4)) {
            ...
            for (int i = 0; i < P.length; i++) {
                if ((P[i].x() <= point.x() + 0.005 && P[i].x() >= point.x() - 0.005) && (P[i].y() <= point.y() + 0.005 && P[i].y() >= point.y() - 0.005)) {
                    Double dis = Math.sqrt(Math.pow(point.x() - P[i].x(), 2) + Math.pow(point.y() - P[i].y(), 2));
                    BB bb = new BB(P[i].x(), P[i].y(), dis);
                    pq.insert(bb);
                    if (pq.size() > k) {
