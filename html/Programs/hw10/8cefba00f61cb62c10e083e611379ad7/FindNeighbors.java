...
public class FindNeighbors {
    ...
    public void findShort(Node<Point2D> start, Point2D point, int k, int level) {
        ...
        pq.insert(maxShort);
        if (level % 2 == 1) {
            if (Math.abs(point.x() - start.coordinate.x()) < maxShort.targetPt.distanceTo(point)) {
                if (start.left != null) findShort(start.left, point, k, level + 1);
                if (start.right != null) findShort(start.right, point, k, level + 1);
            } else {
            ...
            if (Math.abs(point.y() - start.coordinate.y()) < maxShort.targetPt.distanceTo(point)) {
                if (start.left != null) findShort(start.left, point, k, level + 1);
                if (start.right != null) findShort(start.right, point, k, level + 1);
            } else {
