...
public class FindNeighbors {
    ...
    private Node root;
    private Node leaf;
    private Node key;//renew
    public static Node query;
    MaxPQ<Node> knear = new MaxPQ();//renew
    ...
    private class Node implements Comparable<Node> {
        ...
        public int compareTo(Node that) {
          if(this.p.distanceTo(query.p)> that.p.distanceTo(query.p)){return 1;}
          else {return -1;}
    }}
    ...
    public Point2D[] query(Point2D point, int k){
        while(!knear.isEmpty()){knear.delMax();}
        key=new Node(null,null,null,null,false);
        ...
        while(!leaf.equals(root)){//leaf=root end
           //System.out.println(knear.size());
           while(knear.size()>k){knear.delMax();}
           double maxdis=knear.max().p.distanceTo(point);
           ...
           if(cmp(query,leaf.parent)<0){
               ...
               //System.out.println(abdis(query,leaf.parent));
                   if(knear.size()>=k&abdis(query,leaf.parent)>=maxdis){
                       ...
                       leaf=leaf.parent;
                   if(!leaf.equals(root)){
                      while(cmp(leaf,leaf.parent)!=cmp(query,leaf.parent)){
                      leaf=leaf.parent;
        ...
        for(int i =k-1;i>-1;i--){result[i]=new Point2D(knear.max().p.x(),knear.max().p.y());knear.delMax();}
    ...
    public static void main(String[] args) throws Exception {
        ...
        try(BufferedReader br = new BufferedReader(new FileReader(""input10-0.txt""))){
            ...
            e.init(pp);  
            //Point2D q=new Point2D(0.144932,0.938569);
            //Point2D q=new Point2D(0.531440,0.616918);
            Point2D q=new Point2D(0.1354339200,0.7019446863);
            Point2D[] re =e.query(q,10);
            for(int i=0;i<10;i++){
