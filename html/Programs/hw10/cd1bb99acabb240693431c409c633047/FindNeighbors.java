    ...
    private Node findNearest(Node x, Point2D p, Orientation orientation,int k) {
       ...
       Node closet = x;
        ...
        if (first != null) {    
            ...
            if(pq.size()>k){
                if(orientation == Orientation.LeftRight && Math.abs(p.x()-x.p.x())>x.dist){
                    return closet;
                }
                else if(orientation == Orientation.AboveBelow && Math.abs(p.y()-x.p.y())>x.dist){
                    return closet;
                }
            }
...
// Segment to monitor
       ...
       //StdOut.println(elapsedTimeInSec);
}
