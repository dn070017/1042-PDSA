...
public class FindNeighbors {
  ...
  private void printPreOrderRec(Node tree2 ,Point2D point ,int k) {
      
     if (tree2== null) {//if(mode==1){mode=0;} else if(mode==0){mode=1;}
    return ;
     }
     /*if(mode==0){
            if(tree2.compareTo1(point.x())>0)
     ...
     if(mode==0){  
      if(maxpq.size()==k)
         { double dis2=Math.abs(tree2.getValue().x()-point.x());
           double dis3=(tree2.getValue().x()-point.x());
         Node temp =(Node)maxpq.max();
         if(dis2<temp.getdistance(point)){mode=1;
             double dis=tree2.getdistance(point);    
             if(dis<temp.getdistance(point)){maxpq.delMax();maxpq.insert((tree2));//System.out.println(""1"");
             }
             }
         else{ mode=1;// System.out.println(""close1"");
              if(dis3>0){tree2.setRight(null);}
               else{tree2.setRight(null);}
            }
         }
      else{//System.out.println(""2"");
        double dis=tree2.getdistance(point);
        maxpq.insert((tree2));mode=1;}
       }
     else if(mode==1){
        if(maxpq.size()==k)
          { double dis2=Math.abs(tree2.getValue().y()-point.y());
            double dis3=(tree2.getValue().y()-point.y());
         Node temp =(Node)maxpq.max();
         if(dis2<temp.getdistance(point)){mode=0;
             double dis=tree2.getdistance(point);    
             if(dis<temp.getdistance(point)){maxpq.delMax();maxpq.insert((tree2));//System.out.println(""3"");
             }
             }
         else{mode=0;//System.out.println(""close2"");
             if(dis3>0){tree2.setRight(null);}
               else{tree2.setLeft(null);}
            
             }
         }
      else{//System.out.println(""4"");
        double dis=tree2.getdistance(point);
        maxpq.insert((tree2));mode=0;}
  }
           //     System.out.println(dis);
            //     System.out.println(1);
   /*               minpq.insert((tree2));
                  if(tree2.getLeft() == null){break;}
                   tree2=tree2.getLeft();
                   mode=1;
          tree2.compareTo1(point.x())<0){ double dis=tree2.getdistance(point);
          //       System.out.println(dis);
           //       System.out.println(2);
                  minpq.insert(tree2);
                  if(tree2.getRight() == null){break;}
    ...
    /* double dis=tree2.getdistance(point);
     if(maxpq.size()==k)
     { Node temp =(Node)maxpq.max();
     if(dis<temp.getdistance(point)){maxpq.delMax();maxpq.insert((tree2));}
     }
     else{maxpq.insert((tree2));}*/
     
      
       
      //System.out.print(a[i]);
    
      printPreOrderRec(tree2.getLeft(),point,k);
      printPreOrderRec(tree2.getRight(),point,k);
      if(mode==1){mode=0;} else if(mode==0){mode=1;}
      ...
      /*   StdDraw.filledCircle(point.x(), point.y(), 0.01);
       ...
       Node tree2=tree;
        Point2D[] result = new Point2D[k];
         printPreOrderRec(tree2,point,k);
    ...
    //          System.out.println(point[num]);
     ...
     /*    for(int i=0;i<num;i++){   
        ...
        FindNeighbors tree=new FindNeighbors();
         tree.init(pointgo);
         Point2D a=new Point2D(0.531440,0.616918);
