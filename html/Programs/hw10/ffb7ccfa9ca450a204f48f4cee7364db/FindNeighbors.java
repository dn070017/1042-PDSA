...
public class FindNeighbors {
    ...
    public Point2D[] query(Point2D point, int k) {
        ...
        distance(this.tree_2d, point, q, mindistance);
        int num = q.size();
        for (int i = 0; i < num; i++) {
...
//        }
    ...
    private void distance(Node x, Point2D point, Queue<Node> q, double mindistance) {
        if (x == null) {
            return;
        }
        ...
        if (x.distance > mindistance) {
            if (x.level % 2 == 0) {  //判斷要不要走左邊
                if (point.x() <= x.point.x()) {
                    distance(x.left, point, q, mindistance);
                }
            } else {
            ...
            if (x.level % 2 == 0) {  //判斷要不要走右邊
                if (point.x() >= x.point.x()) {
                    distance(x.right, point, q, mindistance);
                }
            } else {
                if (point.y() >= x.point.y()) {
                    distance(x.right, point, q, mindistance);
