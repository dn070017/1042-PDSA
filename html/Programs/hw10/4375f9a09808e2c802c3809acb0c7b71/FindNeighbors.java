...
public class FindNeighbors {
    ...
    public void checkShouldSearch(Node parent, Point2D point){
        ...
        if (nodePQ.size() >= k){
            ...
            if (parent.horizontal && parent.getLeft()!= null && nodePQ.peek().distanceToQuery < abs(parent.getLeft().getValue().y() - point.y()))
                parent.getLeft().shouldSearch = false;
            else if (!parent.horizontal && parent.getLeft()!= null && nodePQ.peek().distanceToQuery < abs(parent.getLeft().getValue().x() - point.x()))
                parent.getLeft().shouldSearch = false;
            
            if (parent.horizontal && parent.getRight()!= null && nodePQ.peek().distanceToQuery < abs(parent.getRight().getValue().y() - point.y()))
                parent.getRight().shouldSearch = false;
            else if (!parent.horizontal && parent.getRight()!= null && nodePQ.peek().distanceToQuery < abs(parent.getRight().getValue().x() - point.x()))
                parent.getRight().shouldSearch = false;
        }
