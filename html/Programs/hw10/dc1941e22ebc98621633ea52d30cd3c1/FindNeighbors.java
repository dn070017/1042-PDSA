...
public class FindNeighbors {
    ...
    private class Node {
        ...
        double verticalDistance(final Node that, final int axis) {
            final double d = axis==0? this.value.x() - that.value.x():this.value.y()-that.value.y();
            return d;
        }
    }
...
//    private double bestDist=1.5;
    ...
    //    private void findKNN(Node testNode,int k,MaxPQ<Node> knn,Node currentNode,int vOrh) {
    private void findKNN(Node testNode, int k, MaxPQ<Node> knn) {

    for (int i = 0; i < pl.size(); i++) {
        pl.get(i).setDist(pl.get(i).value,testNode.value);
        knn.insert(pl.get(i));
...
//
//        Node nextNode = (lOrr < 0) ? currentNode.left : currentNode.right;
...
//            findKNN(testNode,k,knn,nextNode, vOrh+1);
//        }else{
//            currentNode.setDist(currentNode.value,testNode.value);
//            if(currentNode.getDist()<knn.max().getDist()){
//                knn.insert(currentNode);
//                knn.delMax();
//            }
//
//            if(testNode.verticalDistance(currentNode,lOrr)<knn.max().getDist()){
//                findKNN(testNode,k,knn, otherSide,vOrh);
//            }
//        }
