...
public class FindNeighbors {
    ...
    /*private class orientation {
        ...
        public orientation(String t) {
            this.orientation = t;
        }

        private void next() {
            if (this.orientation.equals(""Abovebelow"")) {
                this.orientation = ""Leftright"";
            } else {
                this.orientation = ""Abovebelow"";
            }
        }
    ...
    public Point2D[] query(Point2D p,int k) {
        ...
        findNearest(root, p, k,""Leftright"",pq);
    ...
    private void findNearest(Node x, Point2D p, int k, String orientation, MaxPQ<Event> pq) {
        if (x == null) {
            return;
        }
        pq.insert(new Event(p.distanceTo(x.p),p,x.p));
        ...
        if (left != null && left.rect.distanceTo(p) < pq.max().distance) {
            findNearest(left, p, k,nextorientation, pq);
        }
        if (right != null && right.rect.distanceTo(p) < pq.max().distance) {
            findNearest(right, p, k,nextorientation, pq);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        try (BufferedReader br = new BufferedReader(new FileReader(args[0]))){
            ...
            Point2D[] result = test2.query(point, 3);
            for(int j = 0; j < 3; j++){
