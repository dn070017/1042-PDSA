...
public class FindNeighbors {
    ...
    public void FindKNN(Node pointer, boolean m) {
        ...
        if (yuru.size() < side) {
            yuru.insert(new Pair(tip, pointer.getValue()));
        } else {
            if (yuru.max().distance > tip.distanceTo(pointer.getValue())) {
                yuru.delMax();
                yuru.insert(new Pair(tip, pointer.getValue()));
            }
        }
        ...
        Node near;
        Node far;
        if ((m && (pointer.getValue().x() < tip.x())) || (!m && (pointer.getValue().y() < tip.y()))) {
            far = pointer.getLeft();
        ...
        if (yuru.size() < side) {
            ...
            if (yuru.max().distance > Shortdistance(pointer, m)) {
                FindKNN(far, !m);
            }
