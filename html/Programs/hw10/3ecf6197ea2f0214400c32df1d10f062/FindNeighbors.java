...
public class FindNeighbors {
    ...
    public Point2D[] query(Point2D point , int k){
        MaxPQ<Node> pq = new MaxPQ<Node>(new Comparator<Node>() {
            ...
            public int compare(Node one, Node two) {
                if (one.getDistance() > two.getDistance()) return 1;
                else if (one.getDistance() < two.getDistance()) return -1;
                else return 0;
            }
        });
        QH(pq , root , point , k , Double.MAX_VALUE);
    ...
    private void QH(MaxPQ<Node> pq , Node root , Point2D target , int k , double minDistance) {
        if (root == null) return;
        ...
        QH(pq , root.getNext(targetNode) , target, k , minDistance);

        if (pq.size() < k) {
            QH(pq, root.getOpposite(targetNode), target, k , minDistance);
    ...
    private void getNearestNode(Node root , Point2D target) {
        ...
        if (root.isDependsOnX()) {
            if (Math.abs(root.getX() - target.x()) < nearestDistance)
                getNearestNode(root.getOpposite(targetNode) , target);
        } else {
    ...
    static class Node{
        ...
        public Node getOpposite(Node target) {
            if (this.isLeaf()) return null;
            if (this.isDependsOnX()){
