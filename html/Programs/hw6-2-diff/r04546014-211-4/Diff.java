...
.
    ...
    public static int maxPair(Card[] card) {//回傳最大的pair的最大的牌
        ...
        for (int i = card.length - 1; i > 0; i--) {
            if (card[i].getFace().equals(card[i - 1].getFace())) {
                index = i;
            }
        }
        return index;
    ...
    public static boolean suitCompare(Card[] card) {//檢查兩張卡片的花色是否相同(同花)
        ...
        for (int i = 0; i < card.length-1; i++) {
            if (card[i].getSuit().equals(card[i+1].getSuit())) {
                count++;
            }
        }
    ...
    public static boolean stright(Card[] card) {//判斷是否為順子
        ...
        if (face[card.length - 1] == 14) {//數列中有A的情況
            if(face[0]==2&&face[1]==3&&face[2]==4&&face[3]==5) strightOrNot=true;//判斷是否有A23456或10JQKA
            else if(face[0]==10&&face[1]==11&&face[2]==12&&face[3]==13) strightOrNot=true;
