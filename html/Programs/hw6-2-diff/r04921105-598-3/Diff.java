...
public class Player implements Comparable<Player>{ 
    ...
    public int compareTo(Player that) {
        ...
        if(cardtorank(this.cards)==cardtorank(that.cards)){
            //2pair:
            if (cardtorank(this.cards) ==2){
                if(cards[0].compareTo(this.cards[0])>cards[0].compareTo(that.cards[0])||cards[3].compareTo(this.cards[3])>cards[3].compareTo(that.cards[3]))
                    return 1;   
                else
                    return -1;
            }
            if (cardtorank(this.cards) ==1){
                return 1;
            }
            if (cardtorank(this.cards) ==3){
                return 1;
            }
            if (cardtorank(this.cards) ==4){
                return 1;
            }
            if (cardtorank(this.cards) ==5){
                return 1;
            }
