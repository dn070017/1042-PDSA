    ...
    public boolean twopair(int[] intface, Card[] cards) {
        if (intface[0] == intface[1]) {
            if (intface[3] == intface[4]) {
                thissize = new Card(cards[2].getFace(), cards[2].getSuit());
                return true;
            } 
            else if (intface[2] == intface[3]) {
                thissize = new Card(cards[4].getFace(), cards[4].getSuit());
                return true;
            }
        }
        else if(intface[1] == intface[2]){
            if (intface[3] == intface[4]) {
                thissize = new Card(cards[0].getFace(), cards[0].getSuit());
                return true;
            } 
        }
        return false;
...
.
        ...
        //check that size
        thatsize = new Card(this.cards[4].getFace(),this.cards[4].getSuit());
        if(pair(thatintface, that.cards)){
            thatord =2;
        }
        if (twopair(thatintface, that.cards)) {
            thiatord =3;
        }
