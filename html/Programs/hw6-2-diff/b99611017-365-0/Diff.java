...
public class Player implements Comparable<Player> {
    ...
    public int[] combo(int[] card) {
        ...
        //flush
        if (suitGroup == 5 && sgMax == 1) {

            ans[0] = 1;
            ans[1] = card[card.length - 1];
        }
        //straight
        if (faceGroup == 5 && fgMax == 1) {
            int TF = 0;
            for (int i = 0; i < card.length - 1; i++) {
                if (card[i + 1] % 13 == card[i] % 13 + 1) {
                    TF = 1;
                } else {
                    TF = 0;
                }
            ...
            if (TF == 1) {
                ans[0] = 2;
                ans[1] = card[card.length];
            }
        }
        //two pair
        if (faceGroup == 3 && fgMax == 2) {
            ans[0] = 3;
            for (int i = 0; i < card.length - 1; i++) {
...
.
        int ans = 0;
        int[] a = combo(sortCard(cardLabel(this.cards)));
        int[] b = combo(sortCard(cardLabel(that.cards)));
        if (a[0] < b[0]) {
            ans = 1;
        } else if (a[0] > b[0]) {
            ans = -1;
