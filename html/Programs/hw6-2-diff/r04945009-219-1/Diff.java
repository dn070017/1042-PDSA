public class Player implements Comparable<Player>{ 
    ...
    public boolean fullHouse() {
        if (this.cards[0].getFace() != this.cards[1].getFace()) return false;
        if (this.cards[3].getFace() != this.cards[4].getFace()) return false;
        return !(this.cards[2].getFace() != this.cards[1].getFace() && this.cards[2].getFace() != this.cards[3].getFace());
    }
    ...
    public boolean flush() {
        ...
        for (int i=1; i<5; i++) {
            if (first != this.cards[i].getSuit()) return false;
        }
        return true;
    }
    ...
    public boolean straight() {
        ...
        if (faceNum == 12) {
            int count = 0;
            for (int i=1; i<5; i++) {
                if (this.cards[i].getFace() != faceOrder[faceNum-i]) break; count++;
            }
            if (count==4) return true;
        }
        for (int i=1; i<5; i++) {
            if (this.cards[i].getFace() != faceOrder[faceNum-i]) return false;
        }
        return true;
    }
