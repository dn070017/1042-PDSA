...
.
        ...
        if (c1 < c2) {
            ...
            if (c1 == 6) {
                ...
                if (this.cards[cards.length - 1].getFace().equals(""A"") && that.cards[cards.length - 1].getFace().equals(""A"") && this.cards[cards.length - 2].getFace().equals(""5"") && that.cards[cards.length - 2].getFace().equals(""5"")) {
                    return this.cards[cards.length - 2].compareTo(that.cards[cards.length - 2]);
                } else {
                    return this.cards[cards.length - 1].compareTo(that.cards[cards.length - 1]);//比較倒數第二張牌(這裡會有錯，如果兩副牌都是10JQKA應該先比較A的大小而不是K，但是還要再寫if就很懶)
                }
